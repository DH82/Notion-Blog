...
export const getPosts = async () => {
  ...
  if (
    ...
    for (let i = 0; i < pageIds.length; i++) {
      ...
      // Use Notion page cover as thumbnail (takes priority over manual thumbnail property)
      const pageCover = (pageBlockValue?.format as any)?.page_cover
      if (pageCover) {
        try {
          properties.thumbnail = customMapImageUrl(pageCover, pageBlockValue)
        } catch {
          // fall back to existing thumbnail from database property
        }
      }
